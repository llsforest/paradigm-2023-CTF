        ...
        while True:
            try:
                ...
                async def calculate_lp_token_price(i, res):
                    # 名字，对应地址
                    pool, addr = res
                    # 获取lp的数量，addr.balanceOf(flagCharity)
                    amount = await self.get_balance(block_number, addr, flag_charity)
                    # pool的reverse0，reverse1，totalSupply
                    (
                        token_amount_a,
                        token_amount_b,
                        total_supply,
                    ...
                    # (tokenA的价格*reserve + tokenB的价格*reserve) * lp数量 / totalSupply
                    return (
                ...
                acc += sum(
                    await asyncio.gather(
                        # priceOf(addr) * addr.balanceOf(flagCharity)
                ...
                acc += sum(
                    await asyncio.gather(
                        *[
                            # lp价格为(tokenA的价格*reserve + tokenB的价格*reserve) * lp数量 / totalSupply
